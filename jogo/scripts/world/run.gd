...
## Mapeia o índice (0..11) para a receita do Director.
## 0 = vazia (Sala 1); 1-9 = combate (Salas 2-10); 10 = pré-boss; 11 = chefe.
func _build_for_index(index: int, director: RoomDirector) -> Node2D:
	if index <= 0:
		return director.build_empty_room()
	if index == _BOSS_INDEX:
		return director.build_boss_room()
	if index == _BOSS_INDEX - 1:
		return director.build_combat_room(1)
	return director.build_combat_room((index - 1) % 3 + 1)
